...
import { ExchangeRate } from './exhange-rate.type';
import { SupportedCurrency } from './transaction.type';
...
export const getRate = (date: Date, currency: SupportedCurrency): BigNumber => {
  ...
  if (!exchangeRate) {
    log.error(prefix, 'Failed to load exhange rate', format(date, 'yyyy-MM-dd'), format(exchangeRateDate, 'yyyy-MM-dd'));
    throw new Error();
  }
...
export const loadExchangeRates = () =>
  new Promise<readonly ExchangeRate[]>((resolve, reject) => {
    ...
    soap.createClient('http://www.mnb.hu/arfolyamok.asmx?singleWsdl', { handleNilAsNull: true }, async (error, client) => {
      client.GetExchangeRates(
        ...
        async (error: unknown, response: any) => {
          const rates = parser.parse(response.GetExchangeRatesResult).MNBExchangeRates.Day;
